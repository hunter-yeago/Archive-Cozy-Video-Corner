...
export function Sidebar(props) {
    ...
    const [showDisplay, setShowDisplay] = useState("displaypanel")

    function handleSearchToggle(idx) {
        console.log(idx.target.id);
        setCurrentPanel(idx.target.id);

        if (idx.target.id === currentPanel) {
            toggleShowDisplay();
        }
    }

    function toggleShowDisplay() {
        if (showDisplay === "displaypanel") {
            setShowDisplay("hiddenpanel")
        } else {
            setShowDisplay("displaypanel")
        }
    }
    ...
    return (
        <>
            <div className='outerverticalcontainer'>
                <SearchPanel />
                {/* Inside the Sidebar  */}
                <button className='barbutton'  id="search" onClick={handleSearchToggle}>
                    <svg className= "sidebarsvg" xmlns="http://www.w3.org/2000/svg" fill = "#e3e4e5" viewBox="0 0 24 24" id="search"><g data-name="Layer 2"><path d="m20.71 19.29-3.4-3.39A7.92 7.92 0 0 0 19 11a8 8 0 1 0-8 8 7.92 7.92 0 0 0 4.9-1.69l3.39 3.4a1 1 0 0 0 1.42 0 1 1 0 0 0 0-1.42zM5 11a6 6 0 1 1 6 6 6 6 0 0 1-6-6z" data-name="search"></path></g></svg>
                </button>
                <button className='barbutton' id="info" onClick={handleSearchToggle}>
